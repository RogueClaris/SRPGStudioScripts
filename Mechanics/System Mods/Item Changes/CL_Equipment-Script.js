...
ItemDropListScrollbar.initialize = function () {
	...
	this._list = [ItemControl.getEquippedWeapon(this._unit), ItemControl.getEquippedShield(this._unit), ItemControl.getEquippedArmband(this._unit)]
};
...
var ShieldItemAvailability = defineObject(BaseItemAvailability,
	{
		isItemAllowed: function (unit, targetUnit, item) {
			return item.getTargetAggregation().isCondition(unit);
		},
...
AttackFlow._doAttackAction = function () {
	...
	var shield = ItemControl.getEquippedShield(passive);

	if (isItemDecrement && shield !== null) {
